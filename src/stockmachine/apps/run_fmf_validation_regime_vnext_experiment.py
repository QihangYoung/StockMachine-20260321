...
from stockmachine.risk.regime import MultiSignalBucketScoreRegimeDetector
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a validation-only FMF multi-signal regime vNext experiment."
    )
    parser.add_argument("--data-root", default=None, help="Optional storage root for silver tables.")
    parser.add_argument(
        "--output-root",
        default="artifacts/fmf_validation_regime_vnext_experiment",
        help="Artifact directory for validation-only regime-vNext outputs.",
    )
    ...
    parser.add_argument("--trend-lookback", type=int, default=84)
    parser.add_argument("--relative-lookback", type=int, default=63)
    parser.add_argument("--vol-lookback", type=int, default=42)
    parser.add_argument("--correlation-lookback", type=int, default=63)
    parser.add_argument("--normalization-lookback", type=int, default=126)
    parser.add_argument("--score-smoothing-halflife", type=float, default=10.0)
    parser.add_argument("--risk-on-min-score", type=float, default=0.30)
    parser.add_argument("--defensive-min-score", type=float, default=0.30)
    parser.add_argument("--risk-on-net-threshold", type=float, default=0.02)
    parser.add_argument("--defensive-net-threshold", type=float, default=-0.02)
    parser.add_argument("--equity-duration-shift", type=float, default=DEFAULT_REGIME_SHIFT)
    return parser


def build_validation_regime_state_frame_vnext(
    ...
    *,
    ...
    score_smoothing_halflife: float,
    risk_on_min_score: float,
    defensive_min_score: float,
    risk_on_net_threshold: float,
    defensive_net_threshold: float,
) -> pd.DataFrame:
    detector = MultiSignalBucketScoreRegimeDetector(
        benchmark_column=str(benchmark_column),
        duration_column="duration",
        credit_column="credit",
        inflation_column="inflation_hedge",
        trend_column="trend",
        trend_lookback_windows=int(trend_lookback),
        relative_lookback_windows=int(relative_lookback),
        vol_lookback_windows=int(vol_lookback),
        correlation_lookback_windows=int(correlation_lookback),
        normalization_lookback_windows=int(normalization_lookback),
        score_smoothing_halflife=float(score_smoothing_halflife),
        horizon_sessions=1,
        risk_on_min_score=float(risk_on_min_score),
        defensive_min_score=float(defensive_min_score),
        risk_on_net_threshold=float(risk_on_net_threshold),
        defensive_net_threshold=float(defensive_net_threshold),
    )
    detector_input = bucket_returns.copy()
    detector_input.index = pd.to_datetime(detector_input.index, utc=False)
    detector_input = detector_input.rename_axis("entry_date").reset_index()
    labeled = detector.label_frame(detector_input, date_column="entry_date")
    labeled["overlay_state"] = labeled["regime_label"].replace({"warmup": "neutral"})
    return labeled
...
def run_fmf_validation_regime_vnext_suite(
    bucket_returns: pd.DataFrame,
    *,
    ...
    strategy_runs.append(
        StrategyRunArtifacts(
            strategy_name="rolling_fmf_c2_regime_overlay_vnext",
...
def main(argv: list[str] | None = None) -> int:
    ...
    _require_return_columns(
        ...
        context="FMF regime-vnext validation universe",
    )
    ...
    symbol_returns = slice_return_frame(symbol_returns, start_date=args.start_date, end_date=args.end_date)
    bucket_returns = slice_return_frame(bucket_returns, start_date=args.start_date, end_date=args.end_date)
    ...
    regime_state_frame = build_validation_regime_state_frame_vnext(
        validation_bucket_returns,
        benchmark_column=str(args.benchmark_column),
        trend_lookback=int(args.trend_lookback),
        relative_lookback=int(args.relative_lookback),
        vol_lookback=int(args.vol_lookback),
        correlation_lookback=int(args.correlation_lookback),
        normalization_lookback=int(args.normalization_lookback),
        score_smoothing_halflife=float(args.score_smoothing_halflife),
        risk_on_min_score=float(args.risk_on_min_score),
        defensive_min_score=float(args.defensive_min_score),
        risk_on_net_threshold=float(args.risk_on_net_threshold),
        defensive_net_threshold=float(args.defensive_net_threshold),
    )
    overlay_policy_map = build_regime_overlay_policy_map(
        equity_duration_shift=float(args.equity_duration_shift),
    )
    strategy_runs, regime_state_frame = run_fmf_validation_regime_vnext_suite(
        validation_bucket_returns,
        covariance_config=covariance_config,
        rolling_config=rolling_config,
        benchmark_column=str(args.benchmark_column),
    ...
    overlay_diagnostics = next(
        ...
        if run.strategy_name == "rolling_fmf_c2_regime_overlay_vnext"
    )
    ...
    write_json_artifact(
        ...
        {
            "experiment_family": "fmf_validation_regime_vnext",
...
def _write_report(
    ...
    *,
    ...
    lines = ["# FMF Validation Regime vNext Summary", ""]

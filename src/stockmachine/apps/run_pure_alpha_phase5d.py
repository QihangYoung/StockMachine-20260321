...
DEFAULT_OUTPUT_ROOT = RESEARCH_ROOT / "phase5d_minhold5_simple_turnover_20260427"
DEFAULT_MIN_HOLD_SESSIONS = 5
PLOT_COLORS = {
    "h10_baseline": "#0f766e",
    "h10_short_hybrid": "#2563eb",
    "minhold5_simple_turnover": "#c2410c",
    "spy_raw": "#6b7280",
    "h10_turnover": "#111827",
...
class Phase5DConfig:
    ...
    fixed_turnover_cap: float


def build_phase5d_minhold5_simple_turnover_artifacts(
    *,
    ...
    config = Phase5DConfig(
        label="minhold5_fixed_h10_mean_turnover",
        min_hold_sessions=DEFAULT_MIN_HOLD_SESSIONS,
        fixed_turnover_cap=h10_mean_turnover,
    )

    records = _simulate_simple_turnover_execution(
    ...
    metric["config_fixed_turnover_cap"] = float(config.fixed_turnover_cap)
    ...
    comparison = _build_comparison_curve(h10_curve, records, h10_turnover_curve)
    ...
    metrics_path = output_dir / "phase5d_metrics.csv"
    actions_path = output_dir / "phase5d_action_summary.csv"
    records_path = output_dir / "phase5d_records.csv.gz"
    curve_path = output_dir / "phase5d_comparison_curve.csv"
    turnover_path = output_dir / "phase5d_h10_turnover_curve.csv"
    plot_path = output_dir / "phase5d_comparison_plot.png"
    memo_path = output_dir / "phase5d_minhold5_simple_turnover_memo.md"
    rollup_path = output_dir / "phase5d_rollup.json"
    ...
    rollup = {
        ...
        "config": {
            "label": config.label,
            "min_hold_sessions": int(config.min_hold_sessions),
            "fixed_turnover_cap": float(config.fixed_turnover_cap),
        },
        "h10_mean_turnover": h10_mean_turnover,
        ...
        "method": "min_hold_5_plus_only_simple_fixed_turnover_cap_equal_to_h10_mean_turnover",
...
def _simulate_simple_turnover_execution(
    *,
    ...
    for signal_date in sorted(targets_by_date):
        ...
        for symbol, previous_weight in current_weights.items():
            age = int(current_ages.get(symbol, 0))
        ...
        if raw_turnover > config.fixed_turnover_cap and raw_turnover > 0:
            scale = float(config.fixed_turnover_cap / raw_turnover)
            applied: dict[str, float] = {}
            for symbol in set(current_weights) | set(candidate):
                previous = current_weights.get(symbol, 0.0)
                target = candidate.get(symbol, 0.0)
                weight = previous + (target - previous) * scale
                if abs(weight) > 1e-12:
                    applied[symbol] = float(weight)
            action = "cap_turnover"
        else:
            applied = candidate
            action = "full_target"
        ...
        assumed_cost_bps = turnover * assumed_cost_bps_per_side
        net_return = gross_return - assumed_cost_bps / 10000.0
    ...
    if frame.empty:
        raise ValueError("No daily execution rows built for simple-turnover strategy.")
...
def _build_metrics(
    *,
    ...
    row = pd.DataFrame([metric_row])
    return pd.concat([reference, row], ignore_index=True, sort=False)
...
def _build_comparison_curve(
    ...
    out = h10_curve.merge(
        records[
            ...
            columns={
                "gross_return": "minhold5_simple_turnover__return",
                "equity_gross": "minhold5_simple_turnover__equity",
                "drawdown_gross": "minhold5_simple_turnover__drawdown",
                "rolling_60_gross": "minhold5_simple_turnover__rolling_60_return",
                "turnover": "minhold5_simple_turnover__turnover",
            }
        ),
        on="return_date",
        how="inner",
...
def _plot_comparison(curve: pd.DataFrame, *, output_path: str | Path) -> None:
    ...
    series_specs = [
        ...
        (
            "minhold5_simple_turnover",
            "minhold5_simple_turnover__equity",
            "minhold5_simple_turnover__drawdown",
            "minhold5_simple_turnover__rolling_60_return",
        ),
    ]
    ...
    ax4.plot(dates, curve["minhold5_simple_turnover__turnover"], label="simple_turnover_cap", color=PLOT_COLORS["simple_turnover"], linewidth=1.6)
    ...
    ax4.set_xlabel("Date")
    ax1.set_title("Phase5D Min-Hold-5 + Only Simple Fixed Turnover Cap")
    for ax in axes:
        ax.grid(alpha=0.2)
...
def _memo(
    *,
    ...
    return "\n".join(
        [
            "# Phase5D Min-Hold-5 + Only Simple Fixed Turnover Cap",
            "",
            f"Generated: {_utc_now()}",
            "",
            f"Fixed turnover cap = rolling h10 mean turnover = {h10_mean_turnover:.6f}",
            "",
            "## Metrics",
            "",
            _text_table(display),
            ...
            "## Plot",
            "",
            f"![Phase5D comparison]({plot_path.as_posix()})",
            "",
        ]
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run Phase5D min-hold-5 with only a simple fixed turnover cap."
    )
    ...
    build_phase5d_minhold5_simple_turnover_artifacts(

...
DEFAULT_OUTPUT_ROOT = RESEARCH_ROOT / "phase4af_long_reversal_falling_knife_veto_20260425"
LONG_SELECTORS = (
    DEFAULT_LONG_SCORE,
    "long_reversal_veto_structural_weak_t050",
    "long_reversal_veto_structural_weak_t075",
    "long_reversal_top70_veto_structural_weak_t050",
    "long_reversal_soft_structural_weak_overlay",
)
PORTFOLIO_SPECS = (
...
def build_phase4af_long_reversal_falling_knife_veto_artifacts(
    *,
    ...
    """Test reversal-based long overlays that veto structural falling knives."""
    ...
    memo_path = output_dir / "phase4af_long_reversal_falling_knife_veto_memo.md"
    standalone_daily_path = output_dir / "phase4af_standalone_long_daily_validation.csv"
    standalone_window_path = output_dir / "phase4af_standalone_long_window_summary.csv"
    standalone_bins_path = output_dir / "phase4af_standalone_long_score_bin_summary.csv"
    positions_path = output_dir / "phase4af_beta_matched_positions_validation.csv.gz"
    beta_daily_path = output_dir / "phase4af_beta_matched_daily_validation.csv"
    beta_summary_path = output_dir / "phase4af_beta_matched_summary_validation.csv"
    beta_window_path = output_dir / "phase4af_beta_matched_window_summary_validation.csv"
    skipped_path = output_dir / "phase4af_beta_matched_skipped_validation.csv"
    rollup_path = output_dir / "phase4af_rollup.json"
    ...
    rollup = {
        ...
        "method": "reversal_long_overlay_vetoing_structural_falling_knives",
        "lockbox_status": "validation_only_no_test_window_performance",
        "limitations": [
            "The h10 signal panel keeps legacy *_5d column names; values are h10 labels.",
            "This pass uses only existing 5d/20d/60d price-derived features already present in the panel.",
            "The overlay preserves reversal_5d as the baseline ranking and only deletes or penalizes structurally weak names.",
...
def _add_falling_knife_overlay_scores(panel: pd.DataFrame) -> pd.DataFrame:
    frames = []
    for _, group in panel.groupby(["variant", "session_date"], sort=False):
        frame = group.copy()
        baseline = frame[DEFAULT_LONG_SCORE].astype(float)
        baseline_z = _zscore(baseline.to_numpy(dtype=float))
        momentum_20_down = (-frame["momentum_20d_z"]).clip(lower=0.0).astype(float)
        momentum_60_down = (-frame["momentum_60d_z"]).clip(lower=0.0).astype(float)
        residual_20_down = (-frame["beta_residual_momentum_20d_z"]).clip(lower=0.0).astype(float)
        vol_adjusted_down = (-frame["vol_adjusted_momentum_20d_z"]).clip(lower=0.0).astype(float)
        structural_weakness = (
            momentum_20_down + momentum_60_down + residual_20_down + vol_adjusted_down
        ) / 4.0
        top70_score = baseline >= baseline.quantile(0.70)
        dangerous_050 = structural_weakness > 0.50
        dangerous_075 = structural_weakness > 0.75

        frame["falling_knife_structural_weakness"] = structural_weakness
        frame["long_reversal_veto_structural_weak_t050"] = baseline.where(~dangerous_050)
        frame["long_reversal_veto_structural_weak_t075"] = baseline.where(~dangerous_075)
        frame["long_reversal_top70_veto_structural_weak_t050"] = baseline.where(
            ~(top70_score & dangerous_050)
        )
        soft_raw = baseline_z - 0.75 * structural_weakness
        frame["long_reversal_soft_structural_weak_overlay"] = _zscore(
            np.asarray(soft_raw, dtype=float)
        )
        frames.append(frame)
...
def _memo(
    ...
    *,
    ...
    lines = [
        "# Phase4AF Long Reversal Falling-Knife Veto Memo",
        "",
        f"Generated: {_utc_now()}",
        "",
        "## Question",
        "",
        "Instead of replacing the long selector with a delayed stabilization rule, can we preserve reversal_5d as the main ranking and only veto or penalize the most structurally weak falling knives?",
        "",
        "## Selector Definitions",
        "",
        "- `reversal_5d`: old lead long score.",
        "- `long_reversal_veto_structural_weak_t050`: keep reversal_5d, but drop names with strong 20d/60d/residual weakness > 0.50.",
        "- `long_reversal_veto_structural_weak_t075`: same idea, but only for weakness > 0.75.",
        "- `long_reversal_top70_veto_structural_weak_t050`: apply the 0.50 veto only to the top 30% of same-day reversal candidates.",
        "- `long_reversal_soft_structural_weak_overlay`: keep all names but penalize structurally weak candidates.",
        "",
        ...
        "## Score-Bin Check",
        "",
        "Bins are ranked low-to-high by selector. If the overlay works, the top bins should remain healthier than the structurally weakest bins without flattening the whole curve.",
        "",
        _table(
            _display_bins(
                standalone_bins[
        ...
        "- This pass uses only existing 5d/20d/60d/residual price features already in the panel.",
    ]
    return "\n".join(lines) + "\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run Phase4AF long reversal falling-knife veto diagnostics."
    )
    ...
    result = build_phase4af_long_reversal_falling_knife_veto_artifacts(

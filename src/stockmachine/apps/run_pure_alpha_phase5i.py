...
DEFAULT_OUTPUT_ROOT = RESEARCH_ROOT / "phase5i_simple_long_veto_on_sota_20260427"
...
COMBO_SPECS: tuple[dict[str, str], ...] = (
    ...
    {
        "portfolio": "long_veto_t075",
        "series": "long veto t075",
        "long_score": "long_reversal_veto_structural_weak_t075",
        "short_selector": DEFAULT_SHORT_OVERLAY,
    },
    {
        "portfolio": "long_veto_t050",
        "series": "long veto t050",
        "long_score": "long_reversal_veto_structural_weak_t050",
        "short_selector": DEFAULT_SHORT_OVERLAY,
    },
    {
        "portfolio": "long_top70_veto_t050",
        "series": "long top70 veto t050",
        "long_score": "long_reversal_top70_veto_structural_weak_t050",
        "short_selector": DEFAULT_SHORT_OVERLAY,
    },
    {
        "portfolio": "long_soft_overlay_ref",
        "series": "long soft overlay ref",
...
PLOT_COLORS = {
    ...
    "long veto t075": "#0f766e",
    "long veto t050": "#c2410c",
    "long top70 veto t050": "#7c3aed",
    "long soft overlay ref": "#b45309",
    "SPY raw": "#6b7280",
}
...
def build_phase5i_simple_long_veto_artifacts(
    *,
    ...
    positions_path = output_dir / "phase5i_positions_validation.csv.gz"
    diagnostics_path = output_dir / "phase5i_daily_validation.csv"
    skipped_path = output_dir / "phase5i_skipped_validation.csv"
    ...
    sweep_path = output_dir / "phase5i_sweep.csv"
    metrics_path = output_dir / "phase5i_metrics.csv"
    windows_path = output_dir / "phase5i_window_returns.csv"
    comparison_curve_path = output_dir / "phase5i_comparison_curve.csv"
    plot_path = output_dir / "phase5i_comparison_plot.png"
    memo_path = output_dir / "phase5i_simple_long_veto_memo.md"
    rollup_path = output_dir / "phase5i_rollup.json"
    ...
    rollup = {
        ...
        "method": "simple_long_veto_sweep_on_current_sota",
...
def _plot_comparison(curve: pd.DataFrame, *, output_path: str | Path) -> None:
    ...
    ax3.set_xlabel("Date")
    ax1.set_title("Phase5I Simpler Long Veto Sweep on Current SOTA (Validation Only)")
    for ax in axes:
        ax.grid(alpha=0.2)
...
def _memo(
    *,
    ...
    lines = [
        "# Phase5I Simpler Long Veto Sweep on Current SOTA",
        "",
        ...
        "- Only long-side selector changes: baseline reversal, two hard structural-weak vetoes, one top70 hard veto, and the current soft overlay reference.",
        ...
        "## Plot",
        "",
        f"![Phase5I comparison]({plot_path.as_posix()})",
        "",
    ]
    return "\n".join(lines)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run simpler long-veto candidates on top of the current SOTA short overlay."
    )
    ...
    build_phase5i_simple_long_veto_artifacts(

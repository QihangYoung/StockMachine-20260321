...
DEFAULT_OUTPUT_ROOT = RESEARCH_ROOT / "phase5p_price_falling_knife_overlay_20260506"
DEFAULT_STRICT_ROOT = DEFAULT_OUTPUT_ROOT / "strict_h10_rebuild"
DEFAULT_TURNOVER_PENALTY = 0.005
CURRENT_SOTA_SERIES = "current sota"
BEST_NON_PRICE_SERIES = "np fof low mom60 veto t075"
...
COMBO_SPECS: tuple[dict[str, str], ...] = (
    ...
    },
    {
        ...
        "series": BEST_NON_PRICE_SERIES,
        "long_score": "long_np_fof_low_mom60_veto_t075",
        "short_selector": DEFAULT_SHORT_OVERLAY,
    },
    {
        "portfolio": "price_fk_soft_w050",
        "series": "price fk soft w050",
        "long_score": "long_price_fk_soft_w050",
        "short_selector": DEFAULT_SHORT_OVERLAY,
    },
    {
    ...
    {
        "portfolio": "price_fk_x_np_soft_w075",
        "series": "price fk x np soft w075",
        "long_score": "long_price_fk_x_np_soft_w075",
        "short_selector": DEFAULT_SHORT_OVERLAY,
    },
    {
        "portfolio": "price_fk_x_np_soft_w100",
        "series": "price fk x np soft w100",
        "long_score": "long_price_fk_x_np_soft_w100",
        "short_selector": DEFAULT_SHORT_OVERLAY,
    },
)
...
PLOT_COLORS = {
    ...
    BEST_NON_PRICE_SERIES: "#d97706",
    "price fk soft w050": "#0f766e",
    "price fk soft w075": "#0891b2",
    "price fk soft w100": "#155e75",
    "price fk x np soft w075": "#7c3aed",
    "price fk x np soft w100": "#9333ea",
    "SPY raw": "#6b7280",
}
...
def build_phase5p_price_falling_knife_overlay_artifacts(
    *,
    ...
    positions_path = output_dir / "phase5p_positions_validation.csv.gz"
    diagnostics_path = output_dir / "phase5p_daily_validation.csv"
    skipped_path = output_dir / "phase5p_skipped_validation.csv"
    activation_path = output_dir / "phase5p_price_state_activation_summary.csv"
    ...
    price_exposure = _position_price_state_exposure_summary(positions=positions, panel=panel)
    ...
    sweep = (
        ...
        .merge(price_exposure, on="portfolio", how="left")
    )
    ...
    sweep_path = output_dir / "phase5p_sweep.csv"
    metrics_path = output_dir / "phase5p_metrics.csv"
    windows_path = output_dir / "phase5p_window_returns.csv"
    exposure_path = output_dir / "phase5p_position_price_state_exposure.csv"
    comparison_curve_path = output_dir / "phase5p_comparison_curve.csv"
    plot_path = output_dir / "phase5p_comparison_plot.png"
    memo_path = output_dir / "phase5p_price_falling_knife_overlay_memo.md"
    rollup_path = output_dir / "phase5p_rollup.json"

    sweep.to_csv(sweep_path, index=False)
    metrics.to_csv(metrics_path, index=False)
    windows.to_csv(windows_path, index=False)
    price_exposure.to_csv(exposure_path, index=False)
    ...
    rollup = {
        ...
        "method": "candidate_level_price_falling_knife_soft_penalty_on_current_sota",
        "lockbox_status": "validation_only_no_test_window_performance",
        "limitations": [
            "No test-window performance is computed.",
            "The long gross throttle variant from the Phase5O memo is not included in this first pass.",
            "The price falling-knife score uses existing price-derived panel fields and SEC SIC sectors.",
            "Non-price fragility is used only as a multiplier on price-state penalty severity.",
        ],
    }
...
def _add_price_falling_knife_scores(panel: pd.DataFrame, *, long_variant: str) -> pd.DataFrame:
    ...
    for (variant, session_date), group in panel.groupby(["variant", "session_date"], sort=False):
        ...
        part["price_falling_knife_x_np_score"] = part["price_falling_knife_score"] * (
            0.5 + 0.5 * part["long_np_plus_fund_score"].fillna(0.0).clip(0.0, 1.0)
        )
        if is_long_variant:
            base_z = _zscore_array(baseline.to_numpy(dtype=float))
            part["long_price_fk_soft_w050"] = _zscore_array(
                base_z - 0.50 * part["price_falling_knife_score"].to_numpy(dtype=float)
            )
            ...
            part["long_price_fk_x_np_soft_w075"] = _zscore_array(
                base_z - 0.75 * part["price_falling_knife_x_np_score"].to_numpy(dtype=float)
            )
            part["long_price_fk_x_np_soft_w100"] = _zscore_array(
                base_z - 1.00 * part["price_falling_knife_x_np_score"].to_numpy(dtype=float)
            )
        else:
            ...
            rows.append(
                {
                    ...
                    "mean_price_fk_x_np_score": _safe_mean(
                        subset.get("price_falling_knife_x_np_score", pd.Series(dtype=float))
                    ),
...
def _position_price_state_exposure_summary(
    *,
    ...
    for column in score_columns:
        ...
        factor_source[column] = pd.to_numeric(factor_source[column], errors="coerce").fillna(0.0)
    factors = factor_source[["session_date", "symbol", *score_columns]].drop_duplicates(
        ["session_date", "symbol"]
    )
        ...
        rows.append(
            {
                ...
                "mean_selected_long_price_falling_knife_x_np_score": float(
                    item.get("mean_selected_long_price_falling_knife_x_np_score", np.nan)
                ),
                "mean_selected_long_np_plus_fund_score": float(
                    item.get("mean_selected_long_long_np_plus_fund_score", np.nan)
                ),
                "turnover_penalty": float(turnover_penalty),
                "test_window_used": False,
            }
    ...
    rows.append(
        {
            ...
            "mean_selected_long_price_falling_knife_x_np_score": np.nan,
...
def _plot_comparison(curve: pd.DataFrame, *, output_path: str | Path) -> None:
    ...
    ax1.set_title("Phase5P Price Falling-Knife Long Overlay (Validation Only)")
...
def _memo(
    *,
    ...
    metric_cols = [
        ...
        "mean_selected_long_price_falling_knife_x_np_score",
    ]
    ...
    exposure_cols = [
        ...
        "mean_selected_long_price_falling_knife_score",
        "mean_selected_long_price_falling_knife_x_np_score",
        "mean_selected_long_long_np_plus_fund_score",
    ]
    lines = [
        "# Phase5P Price Falling-Knife Long Overlay",
        "",
        ...
        "- Long variants test candidate-level price falling-knife soft penalties.",
        "- Non-price appears only as a multiplier on penalty severity in the hybrid variants.",
        ...
        "",
        "## Price-State Activation / Coverage",
        "",
        ...
        "## Plot",
        "",
        f"![Phase5P comparison]({plot_path.as_posix()})",
        "",
        "## Read",
        "",
        "- This pass tests the Phase5O mechanism memo's main claim inside the alpha thread: price/state should detect falling-knife setups first.",
        "- A good variant should repair 2015 / 2019 May / 2019 Aug without paying the large always-on long-overlay alpha tax.",
        "- The first pass intentionally omits a long-gross throttle; it keeps the portfolio construction contract identical across variants.",
    ]
    return "\n".join(lines) + "\n"
...
def _long_price_score_columns() -> tuple[str, ...]:
    return (
        "long_price_fk_soft_w050",
        "long_price_fk_soft_w075",
        "long_price_fk_soft_w100",
        "long_price_fk_x_np_soft_w075",
        "long_price_fk_x_np_soft_w100",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Phase5P price falling-knife long overlay.")
    ...
    rollup = build_phase5p_price_falling_knife_overlay_artifacts(

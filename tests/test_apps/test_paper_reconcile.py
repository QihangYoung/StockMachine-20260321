...
def test_paper_reconcile_strategy_project_discovers_project_scoped_artifact_dir(tmp_path, capsys) -> None:
    ...
    artifact_dir = workspace.research_root / "demo-2026-03-22-hist_gbm"
    ...
    ledger.record_run_manifest(
        build_paper_run_manifest(
            ...
            model_name="hist_gbm",
            dry_run=False,
            meta={"strategy_lineage": {"strategy_project": "us_equities_h5"}},
        ).to_record()
    )
    ledger.record_order_decision(
        OrderDecisionRecord(
    ...
    assert exit_code == 0
    assert output["resolved_artifact_dir"] == str(artifact_dir)

...
def test_run_p1_rigor_suite_writes_expected_outputs(tmp_path, monkeypatch, capsys) -> None:
    ...
    def _build_bundle(
        *,
        ...
        assert horizon == 5
        assert strategy_project is None
...
def test_run_p1_rigor_suite_passes_h1_strategy_project_into_bundle_builder(tmp_path, monkeypatch, capsys) -> None:
    class _Bundle:
        predict_start = "2025-01-01"
        horizon = 1
    ...
    workspace = build_strategy_project_paths("us_equities_h1", artifact_root=tmp_path / "artifacts")
    ...
    def _strict_sweep(bundle, *, model_names, output_root, top_k, overlay_config):
            ...
            [
                {
                    ...
                    "top_k": top_k,
                    "horizon": bundle.horizon,
                    "artifacts_dir": str(output_root / "extra_trees"),
                    "sessions": 2,
                    "total_return": 0.1,
                    "annualized_return": 0.09,
                    "annualized_volatility": 0.11,
        ...
        model_dir = output_root / "extra_trees"
        model_dir.mkdir(parents=True, exist_ok=True)
        pd.DataFrame(
            [
                {
    ...
    def _topk(bundle, *, model_names, top_k_values, output_root, overlay_config):
        output_root = Path(output_root)
        output_root.mkdir(parents=True, exist_ok=True)
        frame = pd.DataFrame([{"model": "extra_trees", "top_k": int(top_k_values[0]), "sessions": 1}])
        frame.to_csv(output_root / "summary_metrics.csv", index=False)
        return frame
